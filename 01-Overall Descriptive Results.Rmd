...
`r xfun::numbers_to_words(nrow(dta), cap = TRUE,and = TRUE)` (`r nrow(dta)`) RNs participated in this study from 5 countries (Cyprus: n=226, Spain: n=307, Croatia: n=277, Poland: n=87, and Romania: n=286).
...
dta %>% 
  mutate(
    across(all_of(vars_demo) & where(is.labelled), 
           ~ fct_na_value_to_level(as_factor(.), "(Missing)")
    )
  ) %>% 
...
`r xfun::n2w(n_secC, cap = TRUE, and = TRUE)` RNs (`r n_secC`) completed Section C.   
...
### Skills of hospitals nursing administrators

<br>

- Distribution of responses

The table shows the distribution of responses for the question __Please rate the skill of your hospital Nursing administrators in the following areas__.
...
dta %>% 
  ...
  add_overall() %>%
  add_p(list(all_categorical() ~ "chisq.test") )%>% 
  modify_caption("Please rate the skill of your hospital Nursing administrators in the following areas")
...
### Does your organization have a zero tolerance policy on verbal/ phycical abuse?

```{r}
dta %>% 
  filter(section == "Section C") %>% 
  select(country, tolerance_verbal, tolerance_physical)  |> 
  mutate(across(where(is.labelled), as_factor)) %>%  
  tbl_summary(
    by = country,
    missing = "no"
